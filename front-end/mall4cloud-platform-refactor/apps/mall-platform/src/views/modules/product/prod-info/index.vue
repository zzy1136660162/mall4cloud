      ...
      <div class="info-box">
        ...
        <div class="part-content">
          ...
          <div class="part-form-info">
            <product-details v-model:data-form="dataForm" />
          </div>
        </div>
      </div>
...
const getCategoryBack = (categoryId, shopCategoryId) => {
  ...
  if (shopCategoryId) {
    shopCategoryPage().then(data => {
      ...
      const selectedOfShopCategory = idList(shopCategoryList, shopCategoryId, 'categoryId', 'children').reverse()
...
const formValidation = () => {
  ...
  // 勾选"添加属性图片"但未上传图片时
  if (Data.dataForm.skuList.some((skuItem) => skuItem.imgUrl === '') && Data.dataForm.hasSkuImg === 1) {
    ElMessage({
      message: '请上传属性图片！',
      duration: 1000
    })
  ...
  if (Data.dataForm.skuList.some((skuItem) => skuItem.stock === '' || skuItem.stock === null)) {
    ElMessage({
      message: '商品库存不能为空！',
      duration: 1000
    })
  ...
  if (Data.dataForm.skuList.some((skuItem) => !skuItem.priceFee)) {
    ElMessage({
      message: '商品销售价格必须大于0！',
      duration: 1000
    })
