...
const form = useForm({
  ...
  initialValues: {
    account: props.account ?? localStorage.getItem('mall4cloud_login_account') ?? '',
    password: '',
    remember: localStorage.getItem('mall4cloud_login_account') !== null,
  },
})
const onSubmit = form.handleSubmit((values) => {
  ...
  appAccountStore.login(values).then(() => {
    if (values.remember) {
      localStorage.setItem('mall4cloud_login_account', values.account)
    }
    else {
      localStorage.removeItem('mall4cloud_login_account')
    }
