...
export const useAppMenuStore = defineStore(
  ...
  () => {
        ...
        if (item.children && item.children.length > 0) {
          if (appSettingsStore.settings.menu.mode === 'single') {
            ...
            returnMenus.push({
              ...
              children: convertRouteToMenuRecursive(item.children, item.path),
            })
          }
        }
      })
    ...
    // 递归转换路由为菜单，处理绝对路径子路由（如 /sys-user）
    // parentPath: 当前菜单的完整路径，item.path: 子路由路径（可能以 / 开头）
    function convertRouteToMenuRecursive(routes: RouteRecordRaw[], parentPath: string): MenuRecordRaw[] {
      const returnMenus: MenuRecordRaw[] = []
      routes.forEach((item) => {
        // 子路由路径可能是 /sys-user（绝对），需要与父路径合并
        ...
        if (item.children?.length) {
          menuItem.children = convertRouteToMenuRecursive(item.children, itemPath)
        }
